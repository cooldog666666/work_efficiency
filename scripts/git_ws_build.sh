...
git lfs pull
echo -e "###################\n# build_all image #\n###################"
if [[ $BUILD_COMPONENT =~ .*safe.* || $BUILD_COMPONENT =~ .*image.* ]]
then
    echo "[INFO] build safe first"
    build_all --noincremental safe
fi
echo "[INFO] build $BUILD_COMPONENT"
build_all $BUILD_COMPONENT

return_code=$?

[[ $return_code -eq 0 && $BUILD_COMPONENT =~ .*image.* ]] && archive_image
[[ $return_code -eq 0 && $BUILD_COMPONENT =~ .*image.* ]] && archive_upgrade_package
