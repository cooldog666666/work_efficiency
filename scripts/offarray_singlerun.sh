...
function git_authorization()
{
    ...
    artifactory_user=$CFG_ARTIFACTORY_UERID
    artifactory_password=$CFG_ARTIFACTORY_PASSWORD
...
[[ -n ${ITERATION} ]] || die "ITERATION env variable not set by Jenkins job"
[[ -n ${OFFARRAY_CASES} ]] || die "OFFARRAY_CASES env variable not set by Jenkins job"
...
echo "[INFO] Will run ${OFFARRAY_CASES} for ${ITERATION} times."
...
failures=0
declare -a offarray_cases=(${OFFARRAY_CASES})
for case in "${offarray_cases[@]}"
do
    echo "[INFO] Repeat ${case} for ${ITERATION} times."
    repeat ${ITERATION} "${case}"
    [ "$?" -ne 0 ] && let failures++
done

echo "[RESULT] ${failures} cases failed."
