...
testcase_name = 'TC_VVOL_DR_Block_FastClone'
...
def TC_VVOL_DR_Block_FastClone(storage, io_host, uemcli_host, windows_host, d_testparam, d_storage_object={}):
    ...
    datastore_name = datastores[0].name

    test_disk_size = 20 # GB
    n = int(test_disk_size // 10)
    block_disk_size = [(test_disk_size + i) for i in range(0, 4)]
    ...
    #
    #     LOG.print_step('Create harddisks {} for virtual machine.'.format(block_disk_size))
    ...
    #
    # origin_file_path = './io_config_file/device_write_50comp_0dedup_read_no_offset_0-20g.vdb'
    # updated_file_path = './io_config_file/new_vvol_write_50comp_0dedup_read_no_offset_0-20g.vdb'
    ...
    #
    # origin_file_path = './io_config_file/device_write_25comp_0dedup_read_no_offset_0-20g.vdb'
    # updated_file_path = './io_config_file/new_vvol_write_25comp_0dedup_read_no_offset_0-20g.vdb'
    # vdbench = VDBench(io_host)
    # updated_workload_parameter_file = \
    ...
    # # --- VVOL
    # d_compare_profile = dict(D1)
    # d_compare_profile['used'] = compare_profile(values_to_compare=[10 * n, 10 * n], symbols=['=', '='], deviation=0.1)
    # used, saving = check_vvol_space(test_primary_vvol, d_compare_profile, do_assert=do_assert)
    #
    # # --- CBFS
    # d_compare_profile = dict(D1)
    # d_compare_profile['used'] = compare_profile(values_to_compare=[5 * n, 7.5 * n], symbols=['=', '='], deviation=0.2)
    # d_compare_profile['saving'] = compare_profile(values_to_compare=[5 * n, 2.5 * n], symbols=['=', '='], deviation=0.2)
    # d_compare_profile['nAUsNeededIfNoCompression'] = compare_profile(values_to_compare=[10 * n, 10 * n], symbols=['=', '='], deviation=0.2)
    # d_compare_profile['nDedupMappingPointers'] = compare_profile(values_to_compare=[0, 0], symbols=['=', '='], deviation=0.2)
    # used, saving, nAUsNeededIfNoCompression, nPatternZeroMatched, nPatternNonZeroMatched, nDedupMappingPointers = \
    # check_cbfs_space(test_cbfs, d_compare_profile, do_assert=do_assert)
    #
    ...
    # # --- DATASTORE
    # d_compare_profile = dict(D2)
    # d_compare_profile['used'] = compare_profile(values_to_compare=[5 * n + 7.5 * n], symbols=['>'], deviation=0.1)
    # used, saving, saving_percent, saving_ratio = check_datastore_space(test_datastore, d_compare_profile, do_assert=do_assert)
    #
    # # --- POOL
    # d_compare_profile = dict(D2)
    # d_compare_profile['used'] = compare_profile(values_to_compare=[20 + 5 * n + 7.5 * n], symbols=['>'], deviation=0.1)
    # d_compare_profile['saving'] = compare_profile(values_to_compare=[5 * n + 2.5 * n], symbols=['='], deviation=0.2)
    # d_compare_profile['saving_percent'] = compare_profile(values_to_compare=[0], symbols=['>'], deviation=0.1)
    # d_compare_profile['saving_ratio'] = compare_profile(values_to_compare=[1], symbols=['>'], deviation=0.1)
    # used, saving, saving_percent, saving_ratio = check_pool_space(test_pool, d_compare_profile, do_assert=do_assert)
    ...
    #     if B_to_GB(vvol_size_b) in block_selected_size:
    ...
    #
    # ######
    # LOG.print_step('Take fast clone for the virtual machine.')
    # fast_clone_vm_name = vm_name + '_fastclone'
    # fast_clone_vm(vm_name, fast_clone_vm_name, datastore_name, esx_host_ip, snapshot_name, windows_host)
    #
    # #####
    # LOG.print_step('Check space accounting for related level.')
    #
